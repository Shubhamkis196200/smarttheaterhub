    ...
    <div>
      ...
      <section className="section">
        <div className="container" style={{ maxWidth: 800 }}>
          {query && <p style={{ color: '#6c757d', marginBottom: '2rem' }}>{total} result{total !== 1 ? 's' : ''} for "{q}"</p>}
          {matchedTools.length > 0 && <>
            ...
            {matchedTools.map(t => (
              <Link key={t.slug} to={`/tools/${t.slug}`} className="card" style={{ textDecoration: 'none', color: 'inherit', marginBottom: '0.75rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
                ...
                <div><h3 style={{ fontSize: '0.95rem', fontWeight: 700 }}>{t.name}</h3><p style={{ fontSize: '0.8rem', color: '#6c757d' }}>{t.description}</p></div>
              </Link>
            ))}
          </>}
          {matchedReviews.length > 0 && <>
            ...
            {matchedReviews.map(r => (
              <Link key={r.slug} to={`/reviews/${r.slug}`} className="card" style={{ textDecoration: 'none', color: 'inherit', marginBottom: '0.75rem', display: 'block' }}>
                <h3 style={{ fontSize: '0.95rem', fontWeight: 700 }}>{r.title}</h3><p style={{ fontSize: '0.8rem', color: '#6c757d' }}>{r.excerpt}</p>
              </Link>
            ))}
          </>}
          {matchedBlog.length > 0 && <>
            ...
            {matchedBlog.map(p => (
              <Link key={p.slug} to={`/blog/${p.slug}`} className="card" style={{ textDecoration: 'none', color: 'inherit', marginBottom: '0.75rem', display: 'block' }}>
                <h3 style={{ fontSize: '0.95rem', fontWeight: 700 }}>{p.title}</h3><p style={{ fontSize: '0.8rem', color: '#6c757d' }}>{p.excerpt}</p>
              </Link>
            ))}
          </>}
          {query && total === 0 && <p style={{ textAlign: 'center', color: '#6c757d', padding: '2rem 0' }}>No results found. Try a different search term.</p>}
        </div>
      </section>
    </div>
  )
