        ...
        <div className="container" style={{ maxWidth: 800 }}>
          {post.sections.map((section, i) => (
            <div key={i} style={{ marginBottom: '2.5rem' }}>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '1rem', color: '#1a1a2e' }}>{section.heading}</h2>
              {section.content.map((p, j) => (
                <p key={j} style={{ fontSize: '1.05rem', lineHeight: 1.8, color: '#333', marginBottom: '1.25rem' }}>{p}</p>
              ))}
            </div>
          ))}
        </div>
