          ...
          <div className="grid-2">
            {reviews.map(r => (
              <Link key={r.slug} to={`/reviews/${r.slug}`} className="card" style={{ textDecoration: 'none', color: 'inherit' }}>
                <div style={{ fontSize: '3rem', textAlign: 'center', padding: '1.5rem', background: '#f8f9fa', borderRadius: 8, marginBottom: '1rem' }}>{r.image}</div>
                <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#6c757d' }}>{r.date} · {r.readTime}</span>
                </div>
